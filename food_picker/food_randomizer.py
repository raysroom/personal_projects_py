...
# Goals
"""
Create prompt that lists recipes - DONE
Create the randomizer - DONE
Improve the randomizer - DONE
  improvements:
    - show several randomized items without repeat
    - do not show the list in brackets, instead show a cleaned up string
Create GUI for program 
Pull list from online doc or sharable doc - DONE
...
def main():
    while True:
        try:
            cook_eatout = (
                float(input("Cooking or Eating Out?\nChoose 1 or 2: ").lower().strip())
            )
            if cook_eatout == 1:
                cook()
            elif cook_eatout == 2:
